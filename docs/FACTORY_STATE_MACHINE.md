...
| `sync-factory-state` **[Actions]** | `issues` / `pull_request` label or state change, `schedule` every 5 min, `workflow_dispatch` | — | Mirrors the label set onto the Projects v2 `Status` field; applies/removes the `your-turn` label |
