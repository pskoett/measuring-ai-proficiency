...
When the reviewer has flagged specific issues in the PR, address those first before checking CI. Common reviewer-directed repairs include:

- **Missing closing keyword**: If the reviewer's Critical findings include "impl PR must close its source issue", add `Closes #NN` (with the correct issue number) to the PR body. Find the source issue number from the PR title, branch name, a linked issue, or a `plan-NNN` reference in the body.
