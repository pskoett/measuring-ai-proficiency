...
`spec-refiner` adds a `## Recommended implementer` section to every plan file and applies one `impl:*` label to the source issue matching its recommendation. All four `impl:*` labels auto-route: `impl:copilot` through `assign-to-agent`; `impl:claude-opus` and `impl:claude-sonnet` through `assign-to-user claude[bot]`; `impl:codex` through `assign-to-user codex[bot]`. Claude and Codex Partner Agents must be enabled under Settings > Copilot > Coding agent for the non-Copilot paths to succeed. The human reviews the plan PR; if the recommendation looks wrong they swap the `impl:*` label on the source issue before merging. On merge, `plan-merged-dispatcher` writes the plan checklist onto the source issue body and applies `ready-for-implementation`, which triggers `implementer-dispatcher` to do the actual assignment.
