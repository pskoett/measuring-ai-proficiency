...
For simple, clearly bounded issues, `spec-refiner` skips the plan file and applies `ready-for-implementation` and `impl:copilot` directly. No plan PR, no merge gate. `implementer-dispatcher` picks the issue up immediately. For `impl:copilot` the dispatcher auto-assigns; for other `impl:*` labels it calls `noop` with a comment reminding the human to do the UI assignment.
