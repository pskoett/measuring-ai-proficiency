...
from .scanner import RepoScore, LevelScore
...
class TerminalReporter:
    ...
    def report_single(self, score: RepoScore, output: TextIO = sys.stdout) -> None:
        ...
        print(f"  Overall Level: {_color(level_text, _level_color(score.overall_level))}", 
              file=output)
        print(f"  Overall Score: {_color(f'{score.overall_score:.1f}/100', Colors.BOLD)}", 
              file=output)
        print(file=output)
        ...
        for level_num in sorted(score.level_scores.keys()):
            ...
            print(
                f"    {_color(achieved, achieved_color)} {level_score.name}",
                file=output
            )
            print(
            ...
            if self.verbose and level_score.matched_files:
                ...
                if len(level_score.matched_files) > 5:
                    print(f"        {_color(f'... and {len(level_score.matched_files) - 5} more', Colors.DIM)}", 
                          file=output)
            print(file=output)
    ...
    def report_multiple(self, scores: List[RepoScore], output: TextIO = sys.stdout) -> None:
        ...
        sorted_scores = sorted(scores, key=lambda s: (s.overall_level, s.overall_score), reverse=True)
        ...
        for level in range(5):
            ...
            print(f"    Level {level}: [{bar}] {count} repos ({pct:.1f}%)", file=output)
...
class JsonReporter:
    ...
    def report_multiple(self, scores: List[RepoScore], output: TextIO = sys.stdout) -> None:
        ...
        result = {
            ...
            "average_score": round(
                sum(s.overall_score for s in scores) / len(scores) if scores else 0, 2
            ),
...
class MarkdownReporter:
    ...
    def report_multiple(self, scores: List[RepoScore], output: TextIO = sys.stdout) -> None:
        ...
        total = len(scores)
        for level in range(5):
            count = sum(1 for s in scores if s.overall_level == level)
            pct = count / total * 100 if total > 0 else 0
            print(f"| Level {level} | {count} | {pct:.1f}% |", file=output)
        ...
        sorted_scores = sorted(scores, key=lambda s: (s.overall_level, s.overall_score), reverse=True)
        for score in sorted_scores:
            status = ["No AI context", "Basic", "Comprehensive", "Advanced", "Frontier"][score.overall_level]
            print(
...
class CsvReporter:
    ...
    def report_multiple(self, scores: List[RepoScore], output: TextIO = sys.stdout) -> None:
        ...
        print("repo_name,repo_path,overall_level,overall_score,level_1_coverage,level_2_coverage,level_3_coverage,level_4_coverage", file=output)
