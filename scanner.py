...
from .config import LEVELS, CORE_AI_FILES, QUALITY_INDICATORS, LevelConfig
...
class RepoScanner:
    ...
    def _calculate_overall_level(self, level_scores: Dict[int, LevelScore]) -> int:
        ...
        has_core_file = any(
            any(core in f.path for core in CORE_AI_FILES)
            for f in level_1.matched_files
        )
    ...
    def _calculate_overall_score(self, level_scores: Dict[int, LevelScore]) -> float:
        ...
        Weights files by level importance and substantiveness.
        """
        total_score = 0.0
        max_possible = 0.0
    ...
    def _generate_recommendations(self, score: RepoScore) -> List[str]:
        ...
        if score.overall_level == 1:
            # Check what's missing for Level 2
            ...
            if level_2:
                ...
                if not any('CONVENTIONS' in f.path or 'STYLE' in f.path 
        ...
        if score.overall_level == 2:
            # Check what's missing for Level 3
            ...
            if level_3:
                ...
                if not any('MEMORY' in f.path or 'LEARNINGS' in f.path 
